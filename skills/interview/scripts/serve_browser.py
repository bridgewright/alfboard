...
def main(argv=None) -> int:
    ...
    if not api_key or not agent_id:
        print("ERROR: ELEVENLABS_API_KEY or AGENT_ID is missing from .env.", file=sys.stderr)
        return 2

    try:
        signed = get_signed_url(api_key, agent_id)
    except ModuleNotFoundError as e:
        print(f"ERROR: Missing dependency ({e.name}). Run: uv pip install -r requirements.txt", file=sys.stderr)
        return 3
    except Exception as e:
        print(f"ERROR: Could not obtain a signed URL: {e}", file=sys.stderr)
        return 4

    out = skill_dir / "_samples"
    out.mkdir(parents=True, exist_ok=True)
    # Prefer the maintained browser template and retain the inline fallback.
    ...
    for p in range(args.port, args.port + 12):
        try:
            ...
            continue
    if httpd is None:
        print("ERROR: Could not bind a local port.", file=sys.stderr)
        return 5
